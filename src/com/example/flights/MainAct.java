...
public class MainAct extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        fa = new FAdapter(this, R.layout.item1, Flist);
    ...
    private void parseJSONRespone(JSONObject response) {
        ...
        try {
            ...
            JSONArray entries = responseData.getJSONArray("entries");             //должны выдаваться сервером вместе c даннымии

            for (int count = 0; count < entries.length(); count++) {

                JSONObject anEntry = entries.getJSONObject(count);
                flight aFlight = new flight(anEntry.optString("a1"),  //указать в соответствии с серверной частью
                        anEntry.optString("a2"));
                Flist.add(aFlight);
            }
        } catch (JSONException e) {
            // TODO Auto-generated catch block
